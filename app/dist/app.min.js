!function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a="function"==typeof require&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}for(var i="function"==typeof require&&require,o=0;o<r.length;o++)s(r[o]);return s}({1:[function(require){"use strict";require("jquery"),require("ng-file-upload-shim");var angular=require("angular");require("ng-file-upload"),require("angular-resource"),require("angular-cookies"),require("angular-ui-router"),require("restangular"),require("lodash"),require("angular-ui-tree"),require("angular-bootstrap"),require("textAngularSanitize"),require("textAngular"),require("angular-formly"),require("angular-loading-bar"),require("json-formatter");var app=angular.module("kontApp",["ng","ui.router","ngResource","ngCookies","ui.bootstrap","ui.tree","restangular","textAngular","formly","angularFileUpload","angular-loading-bar","jsonFormatter"]);require("./filters"),require("./directives"),require("./controllers"),app.config(["$stateProvider","$urlRouterProvider","$locationProvider","$httpProvider","RestangularProvider",function($stateProvider,$urlRouterProvider,$locationProvider,$httpProvider,RestangularProvider){$locationProvider.html5Mode(!1),RestangularProvider.setBaseUrl("/api"),$urlRouterProvider.otherwise("/sites"),$stateProvider.state("login",{url:"/login",templateUrl:"partials/login.html",controller:"LoginController"}).state("dataforms",{url:"/dataforms",templateUrl:"partials/dataforms.html",controller:"DataFormListController"}).state("dataforms.detail",{url:"/{formId:[0-9]{0,9}}/edit",templateUrl:"partials/dataformdetail.html",controller:"DataFormDetailController"}).state("dataforms.create",{url:"/create",templateUrl:"partials/dataformcreate.html",controller:"DataFormCreateController"}).state("dataforms.records",{url:"/{formId:[0-9]{0,9}}/records/{pageId:[0-9]{0,9}}",templateUrl:"partials/dataformrecords.html",controller:"DataFormRecordsController"}).state("sites",{url:"/sites",templateUrl:"partials/sitelist.html",controller:"SiteListController"}).state("sites.detail",{url:"/{siteId:[0-9]{0,9}}",templateUrl:"partials/sitedetail.html",controller:"SiteDetailController"}).state("sites.create",{url:"/create",templateUrl:"partials/siteadd.html",controller:"SiteAddController"}).state("sites.pagetree",{url:"/{siteId:[0-9]{0,9}}/pagetree",templateUrl:"partials/pagetree.html",controller:"PageTreeController"}).state("sites.pages",{url:"/{siteId:[0-9]{0,9}}/pages/{pageId:[0-9]{0,9}}",templateUrl:"partials/pagedetail.html",controller:"PageEditController"}).state("users",{url:"/users",templateUrl:"partials/userlist.html",controller:"UserListController"}).state("users.add",{url:"/add",templateUrl:"partials/useradd.html",controller:"UserAddNewController"}).state("users.detail",{url:"/{userId:[0-9]{0,9}}",templateUrl:"partials/userdetail.html",controller:"UserDetailController"}).state("users.detail.credentials",{url:"/credentials",templateUrl:"partials/usercredentials.html",controller:"UserCredController"}).state("users.detail.credentials.chpassword",{url:"/chpassword/{credId}",templateUrl:"partials/usercredchpassword.html",controller:"UserCredPasswordController"}).state("users.detail.authtokens",{url:"/authtokens",templateUrl:"partials/userauthtokens.html",controller:"UserAuthTokenController"}).state("sitemap",{url:"/sitemap/{actionName}",template:"OK",controller:"SitemapController"}).state("prerender",{url:"/prerender/do",template:"OK",controller:"PrerenderController"}),$httpProvider.interceptors.push(["$q","$rootScope","$location",function($q,$rootScope,$location){return{responseError:function(rejection){var status=rejection.status,config=rejection.config,method=config.method,url=config.url;return 401===status?$location.path("/login"):$rootScope.error=method+" on "+url+" failed with status "+status,$q.reject(rejection)}}}]),$httpProvider.interceptors.push(["$q","$rootScope",function($q,$rootScope){return{request:function(config){var useAuthTokenHeader=!0,isRestCall=0===config.url.indexOf("/api");if(isRestCall&&angular.isDefined($rootScope.authToken)){var authToken=$rootScope.authToken;useAuthTokenHeader?config.headers["X-Auth-Token"]=authToken:config.url=config.url+"?token="+authToken}return config||$q.when(config)}}}])}]),app.run(["$rootScope","$location","$cookieStore","Restangular",function($rootScope,$location,$cookieStore,Restangular){$rootScope.$on("$viewContentLoaded",function(){delete $rootScope.error}),$rootScope.hasRole=function(role){return void 0===$rootScope.user?!1:void 0===$rootScope.user.roles[role]?!1:$rootScope.user.roles[role]},$rootScope.logout=function(){delete $rootScope.user,delete $rootScope.authToken,$cookieStore.remove("authToken"),$location.path("/login")};var originalPath=$location.path();$location.path("/login");var authToken=$cookieStore.get("authToken");void 0!==authToken&&($rootScope.authToken=authToken,Restangular.one("user").get().then(function(currentUser){$rootScope.user=currentUser.plain(),$location.path(originalPath)})),$rootScope.initialized=!0}])},{"./controllers":6,"./directives":22,"./filters":23,angular:24,"angular-bootstrap":25,"angular-cookies":26,"angular-formly":27,"angular-loading-bar":28,"angular-resource":29,"angular-ui-router":30,"angular-ui-tree":31,jquery:33,"json-formatter":34,lodash:35,"ng-file-upload":37,"ng-file-upload-shim":36,restangular:38,textAngular:40,textAngularSanitize:39}],2:[function(require,module){"use strict";require("angular");module.exports=function($scope,$state,$stateParams,Restangular){$scope.dataform={},$scope.master={},$scope.sites={},Restangular.all("sites").getList().then(function(list){$scope.sites=list,list&&($scope.dataform.siteId=list[0].id)}),$scope.createNew=function(dataform){Restangular.all("dataforms").post(dataform).then(function(newForm){$state.go("dataforms.detail",{formId:newForm.id},{reload:!0})},function(response){$scope.responseError="Error creating dataform: "+response.statusText,$scope.responseErrorShow=!0})}}},{angular:24}],3:[function(require,module){"use strict";var angular=require("angular");module.exports=function($scope,$state,$stateParams,Restangular){$scope.dataform={},$scope.master={},$scope.sites={},Restangular.all("sites").getList().then(function(list){$scope.sites=list}),Restangular.one("dataforms",$stateParams.formId).get().then(function(data){$scope.dataform=data,$scope.master=angular.copy(data),data.siteId||$scope.sites&&($scope.dataform.siteId=$scope.sites[0].id)}),$scope.update=function(dataform){$scope.master=angular.copy(dataform),dataform.save()},$scope.reset=function(){$scope.dataform=angular.copy($scope.master)},$scope.deleteForm=function(dataform){dataform.remove().then(function(){$state.go("^",$stateParams,{reload:!0})},function(response){$scope.responseError="Error: "+response.statusText,$scope.responseErrorShow=!0})}}},{angular:24}],4:[function(require,module){"use strict";require("angular");module.exports=function($scope,$state,$stateParams,Restangular){$scope.dataforms={},Restangular.all("dataforms").getList().then(function(list){$scope.dataforms=list}),$scope.deleteForm=function(f){Restangular.one("dataforms",f.uuid).remove().then(function(){var idx=$scope.dataforms.indexOf(f);$scope.dataforms.splice(idx,1)},function(response){$scope.responseError="Error removing dataform: "+response.statusText,$scope.responseErrorShow=!0})}}},{angular:24}],5:[function(require,module){"use strict";var angular=require("angular"),_=require("lodash");module.exports=function($scope,$state,$stateParams,Restangular){$scope.records={},$scope.dataObjects={},$scope.pageSize=30,$scope.sortBy="posted",$scope.sortDir="desc",$scope.page=$stateParams.pageId?$stateParams.pageId:0,$scope.pages=[],$scope.totalPages=1,$scope.resultPage={};var pageMaker=function(){var pageNums=_.range(0,$scope.totalPages);$scope.pages=_.map(pageNums,function(pageNum){return{num:pageNum,label:pageNum+1}})},loadPage=function(pageNumber){Restangular.one("dataforms",$stateParams.formId).customGET("records",{page:pageNumber,size:$scope.pageSize,sort:$scope.sortBy+","+$scope.sortDir}).then(function(data){0!==data.numberOfElements&&($scope.page=data.number,$scope.resultPage=data,$scope.totalPages=data.totalPages,pageMaker(),$scope.dataObjects=_.map(data.content,function(rec){return angular.fromJson(rec.data)}),$scope.records=data.content)})};$scope.deleteRecord=function(rec){Restangular.one("dataforms",$stateParams.formId).one("records",rec.id).remove().then(function(){$state.go("dataforms.records",{formId:$stateParams.formId,pageId:$scope.page},{reload:!0})})};var checkPageNum=function(n){return 0>n&&(n=$scope.totalPages-1),n>=$scope.totalPages&&(n=0),0>n&&(n=0),n};$scope.prevPage=function(){var n=checkPageNum($scope.page-1);loadPage(n)},$scope.nextPage=function(){var n=checkPageNum($scope.page+1);loadPage(n)},$scope.gotoPage=function(p){var n=checkPageNum(p);loadPage(n)},loadPage($scope.page)}},{angular:24,lodash:35}],6:[function(require){"use strict";var app=require("angular").module("kontApp");app.controller("DataFormListController",require("./dataformlist")),app.controller("DataFormDetailController",require("./dataformdetail")),app.controller("DataFormCreateController",require("./dataformcreate")),app.controller("DataFormRecordsController",require("./dataformrecords")),app.controller("LoginController",require("./login")),app.controller("PageEditController",require("./pageedit")),app.controller("PageTreeController",require("./pagetree")),app.controller("PrerenderController",require("./prerender")),app.controller("SideNavController",require("./sidenav")),app.controller("SiteAddController",require("./siteadd")),app.controller("SiteDetailController",require("./sitedetail")),app.controller("SiteListController",require("./sitelist")),app.controller("SitemapController",require("./sitemap")),app.controller("UserAddNewController",require("./useradd")),app.controller("UserAuthTokenController",require("./userauthtoken")),app.controller("UserCredController",require("./usercred")),app.controller("UserCredPasswordController",require("./usercredpassword")),app.controller("UserDetailController",require("./userdetail")),app.controller("UserListController",require("./userlist"))},{"./dataformcreate":2,"./dataformdetail":3,"./dataformlist":4,"./dataformrecords":5,"./login":7,"./pageedit":8,"./pagetree":9,"./prerender":10,"./sidenav":11,"./siteadd":12,"./sitedetail":13,"./sitelist":14,"./sitemap":15,"./useradd":16,"./userauthtoken":17,"./usercred":18,"./usercredpassword":19,"./userdetail":20,"./userlist":21,angular:24}],7:[function(require,module){"use strict";module.exports=function($scope,$rootScope,$location,$cookieStore,Restangular){$scope.rememberMe=!0,$scope.responseErrorShow=!1,$scope.responseError="",$scope.login=function(){var credentials={username:$scope.username,password:$scope.password};Restangular.all("user").customPOST(credentials,"authenticate").then(function(tokenContainer){$rootScope.authToken=tokenContainer.token,$scope.rememberMe&&$cookieStore.put("authToken",tokenContainer.token),Restangular.one("user").get().then(function(userData){$rootScope.user=userData,$location.path("/")})},function(response){$scope.responseError=response.data?response.data.message?"Error: "+response.data.message:"Error: "+response.data.error:"Error: "+response.statusText,$scope.responseErrorShow=!0})}}},{}],8:[function(require,module){"use strict";var angular=require("angular"),_=require("lodash");module.exports=function($scope,$state,$stateParams,Restangular,$upload,$interval){$scope.page={},$scope.images=[],$scope.docfiles=[],$scope.embedimages=[],$scope.dataforms=[],$scope.master={},$scope.newPropName="",$scope.newPropContent="",$scope.editorOptions={language:"en"},$scope.loadForms=function(){Restangular.all("dataforms").getList({siteId:$scope.page.siteId}).then(function(list){$scope.dataforms=list})},Restangular.one("pages",$stateParams.pageId).get().then(function(data){$scope.page=data,$scope.master=angular.copy(data)}),$scope.$watch("page.title",function(){if($scope.page.autoName){var txt=$scope.page.title+"",newName=txt.replace(/\W/g,"").toLowerCase();$scope.page.name=newName}}),$scope.update=function(page){$scope.master=angular.copy(page),page.save()},$scope.reset=function(){$scope.page=angular.copy($scope.master)},$scope.isUnchanged=function(page){return angular.equals(page,$scope.master)},$scope.loadImages=function(){Restangular.one("pages",$stateParams.pageId).all("images").getList().then(function(list){$scope.images=list})},$scope.removeImage=function(img){Restangular.one("pages",$stateParams.pageId).one("images",img.id).remove().then(function(){$scope.loadImages()})},$scope.imageUrl=function(img){var l="/att/"+$scope.page.siteId+img.directoryPath+"/"+img.name;return l},$scope.thumbUrl=function(img){return null==img.thumb?"/images/282.gif":"/att/"+$scope.page.siteId+img.thumb.directoryPath+"/"+img.name},$scope.imgSetMain=function(img){Restangular.one("pages",$stateParams.pageId).one("images",img.id).all("setmain").customPOST().then(function(){$scope.loadImages()})},$scope.imgMoveUp=function(img){Restangular.one("pages",$stateParams.pageId).one("images",img.id).all("moveup").customPOST().then(function(){$scope.loadImages()})},$scope.imgMoveDown=function(img){Restangular.one("pages",$stateParams.pageId).one("images",img.id).all("movedown").customPOST().then(function(){$scope.loadImages()})},$scope.onImageSelect=function($files){if($scope.selectedFiles=[],$scope.progress=[],$scope.upload&&$scope.upload.length>0)for(var i=0;i<$scope.upload.length;i++)null!=$scope.upload[i]&&$scope.upload[i].abort();$scope.upload=[],$scope.uploadResult=[],$scope.selectedFiles=$files,$scope.dataUrls=[];for(var it=0;it<$files.length;it++){var file=$files[it];$scope.upload=$upload.upload({url:"/api/pages/"+$stateParams.pageId+"/images",method:"POST",data:{fileNum:it},file:file}).progress(function(evt){$scope.progress[it]=Math.min(100,parseInt(100*evt.loaded/evt.total))}).success(function(data){$scope.uploadResult.push(data),$scope.loadUploadedImages()}).xhr(function(xhr){xhr.upload.addEventListener("abort",function(){},!1)})}};var pollers={},replaceImg=function(newImg){var pos=_.findIndex($scope.images,{id:newImg.id});$scope.images[pos]=newImg};$scope.loadUploadedImages=function(){Restangular.one("pages",$stateParams.pageId).all("images").getList().then(function(list){$scope.images=list,angular.forEach($scope.images,function(img){(null==img.thumb||void 0==typeof img.thumb)&&(angular.isDefined(pollers[img.id])||(pollers[img.id]=$interval(function(){Restangular.one("pages",$stateParams.pageId).one("images",img.id).get().then(function(data){if(null!=data.thumb){replaceImg(data);var imgId=data.id;angular.isDefined(pollers[imgId])&&($interval.cancel(pollers[imgId]),pollers[imgId]=void 0,delete pollers[imgId])}})},1e3)))})})},$scope.$on("$destroy",function(){_.forEach(pollers,function(poller){angular.isDefined(poller)&&($interval.cancel(poller),poller=void 0)}),pollers=void 0}),$scope.onFileSelect=function($files){if($scope.selectedFiles=[],$scope.progress=[],$scope.upload&&$scope.upload.length>0)for(var i=0;i<$scope.upload.length;i++)null!=$scope.upload[i]&&$scope.upload[i].abort();$scope.upload=[],$scope.uploadResult=[],$scope.selectedFiles=$files,$scope.dataUrls=[];for(var it=0;it<$files.length;it++){var file=$files[it];$scope.upload=$upload.upload({url:"/api/pages/"+$stateParams.pageId+"/files",method:"POST",data:{fileNum:it},file:file}).progress(function(evt){$scope.progress[it]=Math.min(100,parseInt(100*evt.loaded/evt.total))}).success(function(data){$scope.uploadResult.push(data),$scope.loadFiles()}).xhr(function(xhr){xhr.upload.addEventListener("abort",function(){},!1)})}},$scope.fileMoveUp=function(img){Restangular.one("pages",$stateParams.pageId).one("files",img.id).all("moveup").customPOST().then(function(){$scope.loadFiles()})},$scope.fileMoveDown=function(img){Restangular.one("pages",$stateParams.pageId).one("files",img.id).all("movedown").customPOST().then(function(){$scope.loadFiles()})},$scope.saveFile=function(df){df.put()},$scope.loadFiles=function(){Restangular.one("pages",$stateParams.pageId).all("files").getList().then(function(list){$scope.docfiles=list})},$scope.loadFiles(),$scope.loadImages(),$scope.loadForms(),$scope.removeFile=function(df){Restangular.one("pages",$stateParams.pageId).one("files",df.id).remove().then(function(){$scope.loadFiles()})},$scope.fileUrl=function(df){var l="/att/"+$scope.page.siteId+df.directoryPath+"/"+df.name;return l},$scope.onEmbedImageSelect=function($files){if($scope.selectedFiles=[],$scope.progress=[],$scope.upload&&$scope.upload.length>0)for(var i=0;i<$scope.upload.length;i++)null!=$scope.upload[i]&&$scope.upload[i].abort();$scope.upload=[],$scope.uploadResult=[],$scope.selectedFiles=$files,$scope.dataUrls=[];for(var it=0;it<$files.length;it++){var file=$files[it];$scope.upload=$upload.upload({url:"/api/pages/"+$stateParams.pageId+"/embed",method:"POST",data:{fileNum:it},file:file}).progress(function(evt){$scope.progress[it]=Math.min(100,parseInt(100*evt.loaded/evt.total))}).success(function(data){$scope.uploadResult.push(data),$scope.loadEmbedImages()}).xhr(function(xhr){xhr.upload.addEventListener("abort",function(){},!1)})}},$scope.loadEmbedImages=function(){Restangular.one("pages",$stateParams.pageId).all("embed").getList().then(function(list){$scope.embedimages=list})},$scope.loadEmbedImages(),$scope.removeEmbedImage=function(emb){Restangular.one("pages",$stateParams.pageId).one("embed",emb.id).remove().then(function(){$scope.loadEmbedImages()})},$scope.embedImageUrl=function(emb){var l="/att/"+$scope.page.siteId+emb.directoryPath+"/"+emb.name;return l},$scope.addProp=function(localScope){var pageProperty={name:localScope.newPropName,content:localScope.newPropContent};Restangular.one("pages",$stateParams.pageId).all("properties").customPOST(pageProperty).then(function(updatedPage){$scope.page.properties=updatedPage.properties,$scope.page.propertyMap=updatedPage.propertyMap,localScope.newPropName="",localScope.newPropContent="",$scope.master.properties=angular.copy(updatedPage.properties)},function(response){$scope.responseError="Error creating property: "+response.statusText,$scope.responseErrorShow=!0})},$scope.saveProp=function(prop){Restangular.one("pages",$stateParams.pageId).one("properties",prop.id).customPUT(prop).then(function(){},function(response){$scope.responseError="Error saving property: "+response.statusText,$scope.responseErrorShow=!0})},$scope.deleteProp=function(prop){Restangular.one("pages",$stateParams.pageId).one("properties",prop.id).remove().then(function(){var idx=$scope.page.properties.indexOf(prop);$scope.page.properties.splice(idx,1)},function(response){$scope.responseError="Error removing property: "+response.statusText,$scope.responseErrorShow=!0})},$scope.reset()}},{angular:24,lodash:35}],9:[function(require,module){"use strict";var angular=require("angular");module.exports=function($scope,$state,$stateParams,Restangular){$scope.data={},Restangular.one("pagetrees",$stateParams.siteId).getList().then(function(list){$scope.data=list}),$scope.treeOptions={accept:function(){return!0},dropped:function(event){var srcPageId=event.source.nodeScope.$modelValue.id,srcParentId=0;null!=event.source.nodeScope.$parentNodeScope&&(srcParentId=event.source.nodeScope.$parentNodeScope.$modelValue.id);var srcIndex=event.source.index,dstParentId=0;event.dest.nodesScope.$parent.$modelValue&&(dstParentId=event.dest.nodesScope.$parent.$modelValue.id);var dstIndex=event.dest.index;(dstIndex!==srcIndex||dstParentId!==srcParentId)&&Restangular.one("pages",srcPageId).all("move").customPOST({srcParentId:srcParentId,dstParentId:dstParentId,srcIndex:srcIndex,dstIndex:dstIndex})}},$scope.reloadTree=function(){Restangular.one("pagetrees",$stateParams.siteId).getList().then(function(list){$scope.data=list})},$scope.editPage=function(scope){var nodeData=scope.$modelValue;$state.go("^.pages",{siteId:$stateParams.siteId,pageId:nodeData.id})},$scope.removePage=function(scope){var nodeData=scope.$modelValue;Restangular.one("pages",nodeData.id).customDELETE().then(function(){scope.remove()},function(response){$scope.responseError="Error removing page: "+response.statusText,$scope.responseErrorShow=!0})},$scope.toggle=function(scope){scope.toggle()},$scope.moveLastToTheBeginning=function(){var a=$scope.data.pop();$scope.data.splice(0,0,a)},$scope.newRootPage=function(){null==$scope.data&&($scope.data=[]);var newPage={siteId:$stateParams.siteId,parentId:0,title:"New page."+($scope.data.length+1),name:"newpage"+($scope.data.length+1),publicPage:!1,placeholder:!1};Restangular.all("pages").customPOST(newPage).then(function(){$scope.reloadTree()},function(response){$scope.responseError="Error creating new root page: "+response.statusText,$scope.responseErrorShow=!0})},$scope.newSubItem=function(scope){var nodeData=scope.$modelValue;null==nodeData.nodes&&(nodeData.nodes=[]);var newPage={siteId:$stateParams.siteId,parentId:nodeData.id,title:nodeData.title+"."+(nodeData.nodes.length+1),name:nodeData.title+(nodeData.nodes.length+1),publicPage:!1,placeholder:!1};Restangular.all("pages").customPOST(newPage).then(function(newPage){nodeData.nodes.push({id:newPage.id,title:newPage.title,nodes:[]})},function(response){$scope.responseError="Error creating new page: "+response.statusText,$scope.responseErrorShow=!0})};var getRootNodesScope=function(){return angular.element(document.getElementById("tree-root")).scope()};$scope.collapseAll=function(){var scope=getRootNodesScope();scope.collapseAll()},$scope.expandAll=function(){var scope=getRootNodesScope();scope.expandAll()}}},{angular:24}],10:[function(require,module){"use strict";module.exports=function($scope,$state,$stateParams,Restangular){Restangular.all("prerender").all("submit").doPOST([{}])}},{}],11:[function(require,module){"use strict";module.exports=function(){}},{}],12:[function(require,module){"use strict";module.exports=function($scope,$state,$stateParams,Restangular){$scope.site={},$scope.createNew=function(site){Restangular.all("sites").post(site).then(function(newSite){$state.go("sites.detail",{siteId:newSite.id},{reload:!0})},function(response){$scope.responseError="Error creating site: "+response.statusText,$scope.responseErrorShow=!0})}}},{}],13:[function(require,module){"use strict";var angular=require("angular");module.exports=function($scope,$state,$stateParams,Restangular){$scope.site={},$scope.master={},$scope.selectedSiteIndex=$stateParams.siteId,$scope.newPropName="",$scope.newPropContent="",Restangular.one("sites",$stateParams.siteId).get().then(function(data){$scope.site=data,$scope.master=angular.copy(data)}),$scope.update=function(site){$scope.master=angular.copy(site),site.save()},$scope.reset=function(){$scope.site=angular.copy($scope.master)},$scope.isUnchanged=function(site){return angular.equals(site,$scope.master)},$scope.deleteSite=function(site){site.remove().then(function(){$state.go("^",$stateParams,{reload:!0})},function(response){$scope.responseError="Error: "+response.statusText,$scope.responseErrorShow=!0})},$scope.addProp=function(){var siteProperty={name:$scope.newPropName,content:$scope.newPropContent};Restangular.one("sites",$stateParams.siteId).customPOST(siteProperty).then(function(updatedSite){$scope.site.properties=updatedSite.properties,$scope.site.propertyMap=updatedSite.propertyMap,$scope.newPropName="",$scope.newPropContent=""},function(response){$scope.responseError="Error creating property: "+response.statusText,$scope.responseErrorShow=!0})},$scope.saveProp=function(prop){Restangular.one("sites",$stateParams.siteId).all(prop.id).customPUT(prop).then(function(){},function(response){$scope.responseError="Error saving property: "+response.statusText,$scope.responseErrorShow=!0})},$scope.deleteProp=function(prop){Restangular.one("sites",$stateParams.siteId).all(prop.name).remove().then(function(){var idx=$scope.site.properties.indexOf(prop);$scope.site.properties.splice(idx,1)},function(response){$scope.responseError="Error removing property: "+response.statusText,$scope.responseErrorShow=!0})}}},{angular:24}],14:[function(require,module){"use strict";module.exports=function($scope,Restangular){Restangular.all("sites").getList().then(function(list){$scope.sites=list})}},{}],15:[function(require,module){"use strict";module.exports=function($scope,$state,$stateParams,Restangular){Restangular.all("sitemaps").all($stateParams.actionName).doPOST([{}])}},{}],16:[function(require,module){"use strict";var angular=require("angular");module.exports=function($scope,$state,$stateParams,Restangular){$scope.user={roleMap:{admin:!1,editor:!1,user:!0},roles:["user"]},$scope.master={},$scope.responseError="",$scope.responseErrorShow=!1,$scope.updateRoles=function(){if($scope.user.roleMap){var roles=[],role="";for(role in $scope.user.roleMap)$scope.user.roleMap[role]&&roles.push(role);$scope.user.roles=roles}},$scope.$watch("user.roleMap.editor",function(){$scope.updateRoles()}),$scope.$watch("user.roleMap.admin",function(){$scope.updateRoles()}),$scope.$watch("user.roleMap.user",function(){$scope.updateRoles()}),$scope.update=function(user){$scope.master=angular.copy(user),Restangular.all("users").post({name:user.name,email:user.email,password:user.password,roles:user.roles}).then(function(){$state.go("^",$stateParams,{reload:!0})},function(response){$scope.responseError="Error: "+response.statusText,$scope.responseErrorShow=!0})},$scope.reset=function(){$scope.user=angular.copy($scope.master)},$scope.isUnchanged=function(user){return angular.equals(user,$scope.master)},$scope.reset()}},{angular:24}],17:[function(require,module){"use strict";module.exports=function($scope,$stateParams,Restangular){Restangular.one("users",$stateParams.userId).getList("authtokens").then(function(tokens){$scope.authtokens=tokens}),$scope.getToken=function(token){Restangular.one("users",$stateParams.userId).one("authtokens",token.uuid).get().then(function(t){$scope.authtoken=t})},$scope.removeToken=function(token){Restangular.one("users",$stateParams.userId).one("authtokens",token.uuid).remove();var idx=$scope.authtokens.indexOf(token);$scope.authtokens.splice(idx,1)}}},{}],18:[function(require,module){"use strict";module.exports=function($scope,$state,$stateParams,Restangular){Restangular.one("users",$stateParams.userId).getList("credentials").then(function(creds){$scope.credentials=creds}),$scope.removeCredential=function(){}}},{}],19:[function(require,module){"use strict";var angular=require("angular");module.exports=function($scope,$state,$stateParams,Restangular){Restangular.one("users",$stateParams.userId).one("credentials",$stateParams.credId).customGET("chpassword").then(function(formdata){$scope.passwordData=formdata,$scope.master=angular.copy($scope.passwordData)}),$scope.responseError="",$scope.responseErrorShow=!1,$scope.master={},$scope.update=function(passwordData){$scope.master=angular.copy(passwordData),Restangular.one("users",$stateParams.userId).one("credentials",$stateParams.credId).all("chpassword").customPUT($scope.passwordData).then(function(){$state.go("^")},function(response){$scope.responseError="Error: "+response.statusText,$scope.responseErrorShow=!0})},$scope.reset=function(){$scope.passwordData=angular.copy($scope.master)},$scope.isUnchanged=function(passwordData){return angular.equals(passwordData,$scope.master)},$scope.reset()}},{angular:24}],20:[function(require,module){"use strict";var angular=require("angular");module.exports=function($scope,$state,$stateParams,Restangular){$scope.user={},$scope.master={},Restangular.one("users",$stateParams.userId).get().then(function(user){$scope.user=user,$scope.master=angular.copy(user)}),$scope.updateRoles=function(){if($scope.user.roleMap){var role,roles=[];for(role in $scope.user.roleMap)$scope.user.roleMap[role]&&roles.push(role);$scope.user.roles=roles}},$scope.$watch("user.roleMap.editor",function(){$scope.updateRoles()}),$scope.$watch("user.roleMap.admin",function(){$scope.updateRoles()}),$scope.$watch("user.roleMap.user",function(){$scope.updateRoles()}),$scope.update=function(user){$scope.master=angular.copy(user),user.save()},$scope.deleteUser=function(user){user.remove().then(function(){$state.go("^",$stateParams,{reload:!0})},function(response){$scope.responseError="Error: "+response.statusText,$scope.responseErrorShow=!0})},$scope.reset=function(){$scope.user=angular.copy($scope.master)},$scope.isUnchanged=function(user){return angular.equals(user,$scope.master)},$scope.reset()}},{angular:24}],21:[function(require,module){"use strict";module.exports=function($scope,Restangular){Restangular.all("users").getList().then(function(list){$scope.users=list})}},{}],22:[function(require){"use strict";var app=require("angular").module("kontApp");app.directive("appVersion",["version",function(version){return function(scope,elm){elm.text(version)}}]),app.directive("passwordValidate",function(){return{require:"ngModel",link:function(scope,elm,attrs,ctrl){ctrl.$parsers.unshift(function(viewValue){return scope.pwdValidLength=viewValue&&viewValue.length>=8?!0:!1,scope.pwdHasLetter=viewValue&&/[A-z]/.test(viewValue)?!0:!1,scope.pwdHasNumber=viewValue&&/\d/.test(viewValue)?!0:!1,scope.pwdValidLength&&scope.pwdHasLetter&&scope.pwdHasNumber?(ctrl.$setValidity("pwd",!0),viewValue):void ctrl.$setValidity("pwd",!1)})}}}),app.directive("ngConfirmClick",function(){return{priority:-1,restrict:"A",link:function(scope,element,attrs){element.bind("click",function(e){var message=attrs.ngConfirmClick;message&&!confirm(message)&&(e.stopImmediatePropagation(),e.preventDefault())})}}}),app.directive("loadingWidget",["requestNotification",function(requestNotification){return{restrict:"AC",link:function(scope,element){element.hide(),requestNotification.subscribeOnRequestStarted(function(){element.show()}),requestNotification.subscribeOnRequestEnded(function(){0===requestNotification.getRequestCount()&&element.hide()})}}}]),app.directive("ensureUnique",["Restangular","$stateParams","$timeout",function(Restangular,$stateParams,$timeout){var checking=null;return{require:"ngModel",link:function(scope,ele,attrs,c){scope.$watch(attrs.ngModel,function(){checking||(checking=$timeout(function(){Restangular.one("pages",$stateParams.pageId).customGET("namecheck",{name:encodeURIComponent(c.$modelValue)}).then(function(){c.$setValidity("unique",!0),checking=null},function(){c.$setValidity("unique",!1),checking=null})},500))})}}}]),app.directive("nxEqualEx",function(){return{require:"ngModel",link:function(scope,elem,attrs,model){attrs.nxEqualEx&&(scope.$watch(attrs.nxEqualEx,function(value){void 0!==model.$viewValue&&""!==model.$viewValue&&model.$setValidity("nxEqualEx",value===model.$viewValue)}),model.$parsers.push(function(value){if(void 0===value||""===value)return model.$setValidity("nxEqualEx",!0),value;var isValid=value===scope.$eval(attrs.nxEqualEx);return model.$setValidity("nxEqualEx",isValid),isValid?value:void 0}))}}})},{angular:24}],23:[function(require){"use strict";var app=require("angular").module("kontApp");app.filter("interpolate",["version",function(version){return function(text){return String(text).replace(/\%VERSION\%/gm,version)}}])},{angular:24}],24:[function(require,module){require("./bower_components/angular/angular.min.js"),module.exports=angular},{"./bower_components/angular/angular.min.js":32}],25:[function(){angular.module("ui.bootstrap",["ui.bootstrap.tpls","ui.bootstrap.transition","ui.bootstrap.collapse","ui.bootstrap.accordion","ui.bootstrap.alert","ui.bootstrap.bindHtml","ui.bootstrap.buttons","ui.bootstrap.carousel","ui.bootstrap.dateparser","ui.bootstrap.position","ui.bootstrap.datepicker","ui.bootstrap.dropdown","ui.bootstrap.modal","ui.bootstrap.pagination","ui.bootstrap.tooltip","ui.bootstrap.popover","ui.bootstrap.progressbar","ui.bootstrap.rating","ui.bootstrap.tabs","ui.bootstrap.timepicker","ui.bootstrap.typeahead"]),angular.module("ui.bootstrap.tpls",["template/accordion/accordion-group.html","template/accordion/accordion.html","template/alert/alert.html","template/carousel/carousel.html","template/carousel/slide.html","template/datepicker/datepicker.html","template/datepicker/day.html","template/datepicker/month.html","template/datepicker/popup.html","template/datepicker/year.html","template/modal/backdrop.html","template/modal/window.html","template/pagination/pager.html","template/pagination/pagination.html","template/tooltip/tooltip-html-unsafe-popup.html","template/tooltip/tooltip-popup.html","template/popover/popover.html","template/progressbar/bar.html","template/progressbar/progress.html","template/progressbar/progressbar.html","template/rating/rating.html","template/tabs/tab.html","template/tabs/tabset.html","template/timepicker/timepicker.html","template/typeahead/typeahead-match.html","template/typeahead/typeahead-popup.html"]),angular.module("ui.bootstrap.transition",[]).factory("$transition",["$q","$timeout","$rootScope",function($q,$timeout,$rootScope){function findEndEventName(endEventNames){for(var name in endEventNames)if(void 0!==transElement.style[name])return endEventNames[name]
