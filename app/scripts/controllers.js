...
.controller('PageEditController', ['$scope', '$state', '$stateParams', 'Restangular', '$upload', '$interval', function ($scope, $state, $stateParams, Restangular, $upload,  $interval) {
  $scope.page = {};
  $scope.images = [];
  $scope.docfiles = [];
  $scope.master = {};
  ...
  var refreshImgData = function(img) {
    ...
    Restangular.one('pages', $stateParams.pageId).one('images', img.id).get().then(function (data) {
      if (data.thumb != null) {
        ...
        console.log("No thumb yet for image: " + img.name);
      }
    });
  };
